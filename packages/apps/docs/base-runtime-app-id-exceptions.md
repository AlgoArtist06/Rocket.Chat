...
`AppObjectRegistry.get('id')` / `this.appId` belongs here. Candidates to re-check during Phases 1–2:
`ModifyDeleter.deleteUsers` (→ `doDeleteUsersCreatedByApp`) and `UserRead.getAppUser(appId?)` — both
today thread an *argument* appId through the host accessor, so confirm the source before deciding the
bucket.
